# structure/metrics/efficiency.py
"""
...
def compute_fractal_efficiency(close: Prices, period: int = 10) -> FloatArray:
    ...
    ----------
    close : Prices
        1D array of closing prices (numeric).
    period : int, default=10
        Lookback window for efficiency calculation.

    Returns
    -------
    FloatArray
        Efficiency values in [0, 1], NaN where insufficient data.

    Raises
    ------
    ValueError
        If `period < 1` or input is not 1D.
    TypeError
...
def compute_consistency_score(close: Prices, window: int = 20) -> FloatArray:
    ...
    window : int, default=20
        Lookback window for consistency calculation.

    Returns
    -------
    FloatArray
        Consistency scores in [0, 1], NaN if insufficient data.

    Raises
    ------
    ValueError
        If `window < 1` or input is not 1D.
    TypeError
        If input is not numeric.
...
def compute_fractal_efficiency_extended(close: Prices) -> Dict[str, np.ndarray]:
    ...
    Dict[str, np.ndarray]: Dictionary containing:
        ...
        - 'fractal_slope': Rate of change of efficiency_10 (first derivative)
        - 'fractal_consistency': Rolling standard deviation of efficiency_10 (lower = more consistent)

    Notes
    -----
    - All outputs are float32 arrays.
    - NaN-padded for warm-up periods.
    - fractal_consistency window = 20 bars
    - Uses np.nanstd for robustness to NaNs
    """
    n = len(close)
    ...
    # === FRACTAL SLOPE: derivative of efficiency_10 ===
    fractal_slope = np.full(n, np.nan, dtype=np.float32)
    if n > 1:
        fractal_slope[1:] = np.diff(eff_10)

    # === FRACTAL CONSISTENCY: rolling std of efficiency_10 ===
    fractal_consistency = np.full(n, np.nan, dtype=np.float32)
    win = 20
    if n >= win:
        # Only compute on valid (non-NaN) sections
        windows = sliding_window_view(eff_10, window_shape=win)
        # Use nanstd to handle any remaining NaNs gracefully
        std_vals = np.nanstd(windows, axis=1)
        fractal_consistency[win - 1:] = std_vals

    return {
        'efficiency_10': eff_10,
        'efficiency_20': eff_20,
        'consistency_20': consistency_20,
